...
class UserApplyView(APIView):
    ...
    def post(self, request):
        ...
        applied_job = JobPost.objects.get(id=job_post)
        new_applied = UserApply.objects.create(user= request.user, jobpost=applied_job)
        return Response({"message": "지원이 완료되었습니다!"}, status = status.HTTP_200_OK)
